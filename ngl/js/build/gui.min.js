...
UI.Input.prototype.setValue=function(a){this.dom.value=a;return this};UI.Input.prototype.setName=function(a){this.dom.name=a;return this};UI.TextArea=function(){UI.Element.call(this);var a=document.createElement("textarea");a.className="TextArea";a.style.padding="2px";a.style.border="1px solid #ccc";a.addEventListener("keydown",function(a){a.stopPropagation()},!1);this.dom=a;return this};UI.TextArea.prototype=Object.create(UI.Element.prototype);UI.TextArea.prototype.getValue=function(){return this.dom.value};
...
UI.Form.prototype.setEnctype=function(a){this.dom.enctype=a;return this};UI.File=function(){UI.Input.call(this);this.dom.type="file";this.dom.multiple=!1;return this};UI.File.prototype=Object.create(UI.Input.prototype);UI.File.prototype.setMultiple=function(a){this.dom.multiple=a;return this};UI.EllipsisText=function(a){UI.Text.call(this,a);this.setWhiteSpace("nowrap");this.setOverflow("hidden");this.setTextOverflow("ellipsis");return this};UI.EllipsisText.prototype=Object.create(UI.Text.prototype);
...
l=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.removeAllComponents()}),k=(new UI.PopupMenu("cogs","Settings","window")).setIconTitle("settings").setMarginLeft("10px"),m=(new UI.Panel).setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginLeft("45px"));a.tasks.signals.countChanged.add(function(a,b){if(0<b)s.add(m);else try{s.remove(m)}catch(c){}});var q=(new UI.Range(1,100,a.viewer.params.clipNear,1)).onInput(function(){a.viewer.setClip(q.getValue(),
r.getValue())}),r=(new UI.Range(1,100,a.viewer.params.clipFar,1)).onInput(function(){a.viewer.setClip(q.getValue(),r.getValue())}),p=(new UI.Range(1,100,a.viewer.params.clipDist,1)).onInput(function(){a.viewer.params.clipDist=p.getValue();a.viewer.requestRender()}),t=(new UI.Range(1,100,a.viewer.params.fogNear,1)).onInput(function(){a.viewer.setFog(null,null,t.getValue(),n.getValue())}),n=(new UI.Range(1,100,a.viewer.params.fogFar,1)).onInput(function(){a.viewer.setFog(null,null,t.getValue(),n.getValue())});
k.addEntry("clip near",q).addEntry("clip far",r).addEntry("clip distance",p).addEntry("fog near",t).addEntry("fog far",n);var s=(new UI.Panel).setClass("Panel Sticky").add(b,g,h,l,k);c.add(s,d);return c};
...
b))});e.add((new UI.SelectionPanel(a.selection)).setMarginLeft("20px").setInputWidth("214px"));var d=(new UI.Button("export")).onClick(function(){var b=new Blob([a.structure.toPdb()],{type:"text/plain"});NGL.download(b,"structure.pdb");p.setMenuDisplay("none")}),h=(new UI.Select).setColor("#444").setOptions(function(){var a={"":"[ add ]"},b;for(b in NGL.representationTypes)a[b]=b;return a}()).onChange(function(){a.addRepresentation(h.getValue());h.setValue("");p.setMenuDisplay("none")}),l=(new UI.Select).setColor("#444").setOptions(function(){var b=
{__AU:"AU"};Object.keys(a.structure.biomolDict).forEach(function(a){b[a]=a});return b}()).setValue(a.structure.defaultAssembly).onChange(function(){a.structure.setDefaultAssembly(l.getValue());a.rebuildRepresentations();p.setMenuDisplay("none")}),k=(new UI.Button("import")).onClick(function(){p.setMenuDisplay("none");var c=["xtc","trr","dcd","netcdf","nc"],d=new NGL.DirectoryListingWidget(b,"Import trajectory",c,function(b){var e=b.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?(NGL.log(b),
a.addTrajectory(b.path),d.dispose()):NGL.log("unknown trajectory type: "+e)});d.setOpacity("0.9").setLeft("50px").setTop("80px").attach()});b.signals.componentAdded.add(c);b.signals.componentRemoved.add(c);var m=(new UI.Select).setColor("#444").onChange(function(){a.superpose(b.compList[m.getValue()],!0);a.centerView();m.setValue("");p.setMenuDisplay("none")});c();var q=(new UI.Button("calculate")).onClick(function(){a.structure.autoSS();a.rebuildRepresentations();p.setMenuDisplay("none")}),r=(new UI.Button("duplicate")).onClick(function(){b.addComponent(new NGL.StructureComponent(b,
a.structure.clone(),{}));p.setMenuDisplay("none")}),p=(new UI.ComponentPanel(a)).setDisplay("inline-block").setMargin("0px").addMenuEntry("PDB file",d).addMenuEntry("Representation",h).addMenuEntry("Assembly",l).addMenuEntry("Trajectory",k).addMenuEntry("Superpose",m).addMenuEntry("SS",q).addMenuEntry("Structure",r).addMenuEntry("File",(new UI.Text(a.structure.path)).setMaxWidth("100px").setWordWrap("break-word"));e.addStatic(p).add(g).add(f);return e};
...
g=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.dispose()});d.addStatic(e).addStatic(f).addStatic(g);(a.parent instanceof NGL.StructureComponent||a.parent instanceof NGL.TrajectoryComponent)&&a.repr.selection instanceof NGL.Selection&&d.add((new UI.SelectionPanel(a.repr.selection)).setMarginLeft("20px").setInputWidth("194px"));var h=(new UI.PopupMenu("bars","Representation")).setMarginLeft("45px").setEntryLabelWidth("110px");h.addEntry("type",new UI.Text(a.repr.type));
...
NGL.TrajectoryComponentWidget=function(a,b){function c(a){h.clear().add(k.setWidth("70px"));k.setRange(-1,a-1);q.setRange(-1,a-1);m.setValue(Math.ceil((a+1)/100));n.step=m.getValue();n.end=a}var d=a.signals,e=a.trajectory,f=(new UI.CollapsibleIconPanel("database")).setMarginLeft("20px"),g=new UI.Panel;d.representationAdded.add(function(a){g.add(new NGL.RepresentationComponentWidget(a,b))});d.disposed.add(function(){v.dispose();f.dispose()});var h=(new UI.Panel).setMarginLeft("10px").setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginRight("69px"));
d.gotNumframes.add(c);d.frameChanged.add(function(a){k.setValue(a);q.setValue(a);h.clear().add(k.setWidth("70px"))});var l=(new UI.EllipsisText(e.name)).setWidth("108px");f.addStatic(l);f.addStatic(h);var k=(new UI.Integer(-1)).setMarginLeft("5px").setWidth("70px").setRange(-1,-1).onChange(function(a){e.setFrame(k.getValue());v.setMenuDisplay("none")}),m=(new UI.Integer(1)).setWidth("30px").setRange(1,1E4).onChange(function(){n.step=m.getValue()}),l=new UI.Panel,q=(new UI.Range(-1,-1,-1,1)).setWidth("197px").setMargin("0px").setPadding("0px").setBorder("0px").onInput(function(a){a=
q.getValue();a!==e.currentFrame&&(e.player&&e.player._running?(e.setPlayer(),e.setFrame(a)):e.inProgress||e.setFrame(a))}),r=(new UI.Select).setColor("#444").setOptions({"":"none",linear:"linear",spline:"spline"}).onChange(function(){n.interpolateType=r.getValue()}),p=(new UI.Integer(5)).setWidth("30px").setRange(1,50).onChange(function(){n.interpolateStep=p.getValue()}),t=(new UI.Integer(50)).setWidth("30px").setRange(10,1E3).onChange(function(){n.timeout=t.getValue()}),n=new NGL.TrajectoryPlayer(e,
m.getValue(),t.getValue(),0,e.numframes),s=(new UI.ToggleIcon(!0,"play","pause")).setMarginRight("10px").setMarginLeft("20px").setCursor("pointer").setWidth("12px").setTitle("play").onClick(function(){n.toggle()});n.signals.startedRunning.add(function(){s.setTitle("pause").setValue(!1)});n.signals.haltedRunning.add(function(){s.setTitle("play").setValue(!0)});l.add(s);l.add(q);var y=(new UI.Select).setColor("#444").setOptions({forward:"forward",backward:"backward"}).onChange(function(){n.direction=
...
u).addEntry("Remove PBC",w).addEntry("Superpose",x).addEntry("Step size",m).addEntry("Interpolation type",r).addEntry("Interpolation steps",p).addEntry("Play timeout",t).addEntry("Play direction",y).addEntry("Play mode",z).addEntry("File",(new UI.Text(e.trajPath)).setMaxWidth("100px").setWordWrap("break-word")).addEntry("Dispose",A);f.addStatic(v);f.add(l);f.add(g);return f};NGL.lastUsedDirectory="";NGL.DirectoryListing=function(a){this.baseUrl=void 0!==a?a:"../dir/";this.signals={listingLoaded:new signals.Signal}};
NGL.DirectoryListing.prototype={constructor:NGL.DirectoryListing,getListing:function(a){var b=this;a=a||"";var c=new THREE.XHRLoader,d=this.baseUrl+a;THREE.Cache.remove(d);c.load(d,function(c){c=JSON.parse(c);b.signals.listingLoaded.dispatch(a,c)})},getFolderDict:function(a){var b={"":""},c=[];(a||"").split("/").forEach(function(a){c.push(a);b[c.join("/")]=a});return b}};
NGL.DirectoryListingWidget=function(a,b,c,d,e){function f(a,b,c,d,e){return(b=Math,c=b.log,d=1E3,e=c(a)/c(d)|0,a/b.pow(d,e)).toFixed(2)+String.fromCharCode(160)+(e?"kMGTPEZY"[--e]+"B":"Bytes")}var g=new NGL.DirectoryListing(e);a=g.signals;var h=new UI.OverlayPanel;e=(new UI.Panel).setBorderBottom("1px solid #555").setHeight("30px");var l=(new UI.Panel).setMarginTop("10px").setMinHeight("100px").setMaxHeight("500px").setOverflow("auto"),k=(new UI.Select).setColor("#444").setMarginLeft("20px").setWidth("").setMaxWidth("200px").setOptions(g.getFolderDict()).onChange(function(){g.getListing(k.getValue())});
e.add(new UI.Text(b||"Directoy listing"));e.add(k);e.add((new UI.Icon("times")).setCursor("pointer").setMarginLeft("20px").setFloat("right").onClick(function(){h.dispose()}));h.add(e);h.add(l);a.listingLoaded.add(function(a,b){NGL.lastUsedDirectory=a;l.clear();k.setOptions(g.getFolderDict(a)).setValue(a);b.forEach(function(a){var b=a.path.split(".").pop().toLowerCase();if(!c||a.dir||-1!==c.indexOf(b)){var e;a.dir?(b="folder-o",e=a.name):(b="file-o",e=a.name+String.fromCharCode(160)+"("+f(a.size)+
")");b=(new UI.Panel).setDisplay("block").add((new UI.Icon(b)).setWidth("20px")).add(new UI.Text(e)).onClick(function(){a.dir?g.getListing(a.path):d(a)});a.restricted&&b.add((new UI.Icon("lock")).setMarginLeft("5px"));l.add(b)}})});g.getListing(NGL.lastUsedDirectory);return h};
